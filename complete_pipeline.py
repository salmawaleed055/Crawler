...
# ID: 900232658
# API-based version using data.gov CKAN API - COMPLETE END-TO-END PIPELINE
...
MAX_PAGES = 2
ROWS_PER_PAGE = 1000  # 2 calls × 1000 rows = 2,000 datasets
...
# ============================================================
# STAGE 1: FETCH DATASETS VIA API
# ============================================================
...
def fetch_datasets_via_api():
    """
    Fetch datasets using the data.gov CKAN API instead of web scraping.
    More efficient, faster, and more reliable than HTML parsing.
    """
    datasets = []
...
def extract_dataset_info(package):
    """
    Extract relevant dataset information from API response.
    Maps CKAN API fields to our desired output format.
    """
    ...
    # Extract formats (file types available)
    formats = []
    ...
    # Extract license (as output by API, no replacement)
    license_info = "N/A"
    ...
    # Extract maintainer and maintainer email
    ...
    # Extract description (notes field)
    description = "N/A"
    if package.get('notes'):
        description = package['notes'].strip()[:500]  # Limit to 500 chars
    ...
    if package.get('tags'):
        ...
        if topic_list:
            topic = ", ".join(topic_list[:10])  # Limit to first 10 tags
...
# ============================================================
# STAGE 3: GENERATE CSV FILES
# ============================================================
print("\n[STAGE 3/3] GENERATING CSV FILES FOR DATABASE")
print("-" * 70)

# [3.1] Create ORGANIZATIONS table with full metadata
print("\n[3.1/3.7] Creating ORGANIZATIONS table...")
organizations_dict = {}

# Extract organization data from datasets
for _, row in df_datasets.iterrows():
    ...
    if org_name and org_name != 'N/A' and org_name not in organizations_dict:
        organizations_dict[org_name] = {
            ...
            'organization_type': 'Government'  # Default based on data.gov being government data
        }
...
# [3.4] Create DATASET_TAGS junction table
